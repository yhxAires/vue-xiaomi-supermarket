  ...
  <header class="top-container">
    <div class="top-bar clearfix">
      <div class="topbar-nav">
        <ul class="nav-wrap">
          <li class="nav clearfix" v-for="(item,index) in topbarlist">
            <a :href="item.sourceUrl" target="_blank" class="nav-name">{{item.name}}</a>
      ...
      <div class="topbar-cart">
        <div class="cart" @mouseenter="entermouse" @mouseleave="leavemouse">
          <i class="fa fa-cart-arrow-down icon-cart"></i>
          <a href="http://static.mi.com/cart/">
            购物车(<span>0</span>)
          </a>
        </div>
        <transition name="fade"><div class="cart-list" v-show="cartStatus" >
          购物车中还没有商品，赶快选购吧！
        </div></transition>
      </div>
    </div>
